-- R8 SOC II quick wins (re-apply with column name fixes for client_profiles)
BEGIN;

CREATE OR REPLACE FUNCTION public.tg_audit_pipeline_mutations()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp
AS $$
DECLARE v_user uuid := auth.uid(); v_account uuid; v_user_name text;
BEGIN
  ...
  VALUES (v_account, v_user, COALESCE(v_user_name, 'System'),
...
CREATE OR REPLACE FUNCTION public.tg_audit_logs_immutable() RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'audit_logs are append-only (SOC II Type II requirement)'; END $$;
...
AS $$ SELECT get_user_role_in_account(auth.uid(), p_account) IN ('owner', 'admin', 'attorney') $$;
...
-- Column-level PII gating on client_profiles (using REAL column names: dob not date_of_birth)
REVOKE SELECT(a_number, phone, mobile_phone, dob, ssn_last4) ON public.client_profiles FROM authenticated;

CREATE OR REPLACE FUNCTION public.user_can_see_pii(p_account uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp
AS $$ SELECT get_user_role_in_account(auth.uid(), p_account)
...
GRANT SELECT(id, account_id, first_name, last_name, email) ON public.client_profiles TO authenticated;
...
DO $$ BEGIN
  BEGIN ALTER TABLE public.case_tasks VALIDATE CONSTRAINT case_tasks_one_level_only;
...
ALTER TABLE public.account_members ADD CONSTRAINT custom_permissions_no_escalation
  CHECK (NOT (role IN ('paralegal', 'assistant', 'readonly') AND (
