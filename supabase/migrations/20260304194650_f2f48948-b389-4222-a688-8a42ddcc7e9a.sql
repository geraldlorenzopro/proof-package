...
-- 1. Delete all existing sessions to start clean
DELETE FROM app_active_sessions;

-- 2. Add unique constraint to prevent duplicate user sessions per app per account
ALTER TABLE app_active_sessions ADD CONSTRAINT unique_user_app_account UNIQUE (account_id, app_id, user_id);

-- 3. Replace acquire_app_seat with fixed version that handles UPSERT properly
CREATE OR REPLACE FUNCTION public.acquire_app_seat(_user_id uuid, _app_slug text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Check if user already has a session (upsert heartbeat)
  ...
  IF _max_seats = 0 THEN
    ...
    ON CONFLICT (account_id, app_id, user_id) DO UPDATE SET last_heartbeat = now()
