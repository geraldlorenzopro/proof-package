...
BEGIN
  ...
  -- ═══ CASE VELOCITY: avg days per stage transition ═══
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::jsonb) INTO _case_velocity
  FROM (
    SELECT 
      h.to_stage as stage,
      ...
      )) / 86400)::numeric, 1) as avg_days_in_stage
  ...
  -- ═══ TEAM PRODUCTIVITY: cases per assigned member ═══
  ...
  -- ═══ PIPELINE DISTRIBUTION: cases per stage ═══
  ...
  -- ═══ MONTHLY TREND: cases opened/closed per month (last 6 months) ═══
